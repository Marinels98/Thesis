...
class MLP(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        #input layer -nn.linear represents a fully connected (linear) layer
        self.input = nn.Linear(28 * 28 * 3, 100, dtype=torch.float32)
        #hidden layer
        self.h1    = nn.Linear(100, 100, dtype=torch.float32)
        self.h2    = nn.Linear(100, 16, dtype=torch.float32)
        #self.h3    = nn.Linear(100, 100, dtype=torch.float32)
        #output layer
        self.head  = nn.Linear(16, 10, dtype=torch.float32)
