...
class TV_RESNET18(nn.Module):
    def __init__(self,*args, **kwargs):
        ...
        self.to(self.device)
    def backbone2(self, x: torch.Tensor):
        # Extract features from the ResNet-18 backbone
        ...
        # Perform average pooling and pass through the final classification layer
    ...
    def forward(self, x):
        # Process input data through the backbone
        x = self.backbone(x)
            ...
            with torch.no_grad():
                for inputs, labels, bias_labels in dataloader:
                    ...
                    for x, l, b in zip(inputs, labels, bias_labels):
                        ...
                        features.append(self.backbone2(x).flatten().cpu().numpy())
    ...
    def train_model_iter(
        ...
        early_stopping = EarlyStopping(patience=30, epsilon=1e-6)
        self.train()
                    ...
                    for i, l in enumerate(labels):
                        if rand is False:            
                            if ground_truth is False:
                                with torch.no_grad():
                                    if epoch==0:
                                        features = self.backbone2(inputs[i]).cpu().numpy()
...
if __name__ == '__main__':
    # Create an instance of TV_RESNET18 with the desired number of classes
    num_classes = 1000  # You can change this to the number of classes in your task
    model = TV_RESNET18()
    print(model)

    # Example input tensor (batch size, channels, height, width)
    input_tensor = torch.randn((1, 512, 1, 1)).to("cuda")
